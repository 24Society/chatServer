        ...
        try:
            ...
            if message['cmd'] == 'message':
                ...
                with open(f'groups\\{message["opt"][0]}.grp', 'a', encoding='utf-8') as f:
                    f.write(f'[{now.data['username']}(id:{now.uid})] {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n')
            ...
            if message['cmd'] == 'get_result':
                tmp = groupList[message['opt'][0]]
                tmp.check_date()
                _client_socket.send(('info ' + ' '.join(str(i) for i in tmp.votes.data['last_result'])).encode('utf-8'))
                continue
        except KeyError:
            break
        except ConnectionResetError:
